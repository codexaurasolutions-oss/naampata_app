...
export default function ReviewsScreen({ navigation }: any) {
  ...
  // Calculate stats manually for UI
  const avgRating = reviews.length ? (reviews.reduce((acc: number, cur: any) => acc + cur.rating, 0) / reviews.length).toFixed(1) : '0.0';
        ...
        {isLoading ? (
          ...
          reviews.map((review: any, index: number) => (
            <View key={review.id || index} className="bg-white rounded-3xl p-5 mb-4 border border-slate-100 shadow-sm">
              <View className="flex-row justify-between items-start mb-3">
                <View className="flex-row items-center flex-1">
                  <View className="w-10 h-10 bg-blue-50 rounded-full items-center justify-center">
                    <Text className="font-black text-blue-500 text-lg">{review.user?.name?.charAt(0) || 'U'}</Text>
                  </View>
                  <View className="ml-3">
                    <Text className="font-bold text-[#112D4E] text-base">{review.user?.name || 'Anonymous'}</Text>
                    <Text className="text-slate-400 text-xs">{review.createdAt || 'Just now'}</Text>
                  </View>
                </View>
              ...
              <Text className="text-slate-700 leading-relaxed mb-4">{review.comment}</Text>
